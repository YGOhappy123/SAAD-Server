...
namespace milktea_server.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        [Authorize(Roles = "Admin")]
        [HttpPost("")]
        public async Task<IActionResult> CreateNewAdmin([FromBody] CreateAdminDto createAdminDto, [FromQuery] string locale)
        {
            ...
            var result = await _userService.CreateNewAdmin(createAdminDto, int.Parse(authUserId!), locale);
            if (!result.Success)
            {
                return StatusCode(result.Status, new ErrorResponseDto { Message = result.Message });
            }
